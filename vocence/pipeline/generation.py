...
async def submit_sample_to_api(
    ...
    try:
        ...
        try:
            # Submit each participant result as a sample
            for hotkey, result in participant_results.items():
                evaluation = result.get("evaluation", {})
                await client.submit_sample(
                    sample_id=sample_id,
                    miner_hotkey=hotkey,
                    s3_bucket=AUDIO_SAMPLES_BUCKET,
                    s3_prefix=sample_id,
                    wins=evaluation.get("generated_wins", False),
                    prompt=prompt_text,
            ...
            emit_log(f"Sample {sample_id} submitted to API ({len(participant_results)} results)", "info")
            return True
        finally:
            await client.close()
...
async def generate_samples_continuously(
    ...
    while True:
        ...
        try:
            ...
            description = await generate_description_async(openai_client, audio_path)
            ...
            for hotkey, participant_info in successful_participants.items():
                ...
                emit_log(f"Eval: {result_str} | conf={comparison['confidence']}% | {reasoning_short}", "info")
            
            # 7. Build metadata (full audio as task, no clip/segment fields)
            metadata = {
                "evaluation_id": evaluation_id,
            ...
            # 9. Save to database (if configured)
            await submit_sample_metadata(evaluation_id, metadata, participant_results)
            
            # Summary
            round_duration = time.time() - round_start
            wins = sum(1 for p in participant_results.values() if p["evaluation"]["generated_wins"])
            emit_log(f"Round #{round_num} complete: {wins}/{len(participant_results)} wins, {round_duration:.1f}s total", "success")
