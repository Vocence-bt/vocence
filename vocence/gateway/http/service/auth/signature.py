...
Implements SS58 signature verification for validator requests.
"""
...
class RequestVerifier:
    ...
    async def validate_request(
        ...
        # Verify signature
        try:
            # Create message to verify: hash of (body + timestamp)
            message = self._build_message(body, timestamp)
    ...
    def _build_message(self, body: bytes, timestamp: str) -> bytes:
        ...
        """
        # Hash the body to create a fixed-size message
...
async def verify_admin_signature(
    ...
    hotkey = await verify_validator_signature(
        request, x_validator_hotkey, x_signature, x_timestamp
    )
...
def sign_request_body(keypair: Keypair, body: bytes, timestamp: str) -> str:
    ...
    message = verifier._build_message(body, timestamp)
