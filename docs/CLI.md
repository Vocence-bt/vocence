...
2. **Weight setting** (foreground): every `CYCLE_LENGTH` blocks, reads scores from the validator’s S3 bucket, picks a leader (winner-take-all + threshold), sets weights on chain.
...
**Requires:** `CHUTES_AUTH_KEY`, `OPENAI_AUTH_KEY`, wallet (e.g. `COLDKEY_NAME`, `HOTKEY_NAME`), chain config, corpus + validator Hippius credentials, API URL (`API_URL`).
...
**Endpoints:** participants, evaluations, metrics, blocklist, status. Background workers: participant validation, metrics calculation. Uses DB and env for service host/port.
...
**Requires:** Wallet, chain config, validator Hippius client (read samples bucket), API for participant list.
...
**Network and subnet:** By default the command uses `NETWORK` / `NETUID` (or `CHAIN_NETWORK` / `SUBNET_ID`) from your `.env`. Default is mainnet: `NETWORK=finney`, `NETUID=102`. For testnet set `NETWORK=test` and `NETUID=XXX` in `.env` (replace XXX with the testnet subnet id), or pass `--network finney --netuid 102` to override for this run.

**Usage:**

```bash
vocence miner commit --model-name <repo> --model-revision <sha> --chute-id <id>
vocence miner commit --model-name user/model --model-revision abc123 --chute-id chute-xxx --network finney --netuid 102
```
...
| `--netuid`        | No       | Subnet ID (default mainnet 102; testnet set via .env). Overrides .env. |
